...
TEST_P(RegexpTest, ChainQuantifiers) {
  ...
  EXPECT_THAT(Match(pattern, "abb"), IsOkAndHolds(std::nullopt));
  EXPECT_THAT(Match(pattern, "aabb"), IsOkAndHolds(std::nullopt));
  EXPECT_THAT(Match(pattern, "aaabb"), IsOkAndHolds(Optional(ElementsAre("aaabb"))));
...
TEST_P(RegexpTest, Search) {
  ...
  ASSERT_OK(status_or_pattern);
  auto const& pattern = status_or_pattern.value();
  EXPECT_THAT(Match(pattern, "lorem ipsum dolor sic amat"),
              IsOkAndHolds(Optional(ElementsAre("lorem ipsum dolor sic amat"))));
  EXPECT_THAT(Match(pattern, "lorem ipsum dooolor sic amat"),
              IsOkAndHolds(Optional(ElementsAre("lorem ipsum dooolor sic amat"))));
