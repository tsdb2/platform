...
class lock_free_hash_set
    : private internal::lock_free_container::RawLockFreeHash<Key, void, Hash, Equal, Allocator> {
 private:
  using Base = internal::lock_free_container::RawLockFreeHash<Key, void, Hash, Equal, Allocator>;
 ...
 public:
  ...
  lock_free_hash_set(std::initializer_list<value_type> const init, Allocator const &alloc)
      : lock_free_hash_set(init, Hash(), Equal(), alloc) {}

  lock_free_hash_set(std::initializer_list<value_type> const init, Hash const &hasher,
  ...
  // Swaps the content of two hash sets. All existing iterators are invalidated. This algorithm is
  // not lockless.
  void swap(lock_free_hash_set &other) noexcept { Base::Swap(other); }
  friend void swap(lock_free_hash_set &lhs, lock_free_hash_set &rhs) noexcept { lhs.swap(rhs); }
};
